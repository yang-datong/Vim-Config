...
if [ ! -f "python-3.12.2-embed-amd64.zip" ];then
...
if [ ! -d "$D" ];then
	mkdir "$D"
	unzip python-3.12.2-embed-amd64.zip -d "$D"
fi
...
#设置python库的搜索路径
echo "./Lib/site-packages/" >> "$HOME/.wine/drive_c/Program Files/Python3/python312._pth"
...
wine python.exe -m pip install yara keystone-engine six
