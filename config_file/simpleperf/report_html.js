...
// --- MODIFICATION 1: Define a global set of function names to highlight ---
const gHighlightedFunctionNames = new Set([
// ------------- 8  bit -------------------
    'cdef_brow_8bpc',
		'decode_b',
		'splat_refmvs',
        ...
        return createPromise()
                    ...
                    for (let [j, thread] of process.threads.entries()) {
                        ...
                        for (let [k, lib] of thread.libs.entries()) {
                            ...
                            for (let [t, func] of lib.functions.entries()) {
                                ...
                                //  TODO YangJing 添加函数名字高亮 <26-01-09 14:23:31> // 
                                let funcName = getFuncName(func.f);
                                funcName = funcName.replace(/^dav1d_/, '');
                                if (gHighlightedFunctionNames.has(funcName))
                                    funcName += '*'; // Append asterisk

                                data.push([totalValue, selfValue, func.c[0], processName,
                                           threadName, libName, funcName, key])
                           }
                        }
                    }
                }
            ...
            .then(wait(() => {
                ...
                let dataTable = table.DataTable({
                    ...
                    //  TODO YangJing 高亮函数名字 <26-01-09 14:32:45> // 
                    createdRow: function(row, data, dataIndex) {
                        // data[6] corresponds to the 'Function' column
                        if (data[6] && data[6].endsWith('*')) {
                            $(row).css('color', 'red'); // Highlight the row in red
                        }
                    },
        ...
        .then(wait(() => {
            ...
            for (let i = 0; i < newCount; ++i) {
                ...
                if (i >= this.flamegraphs.length) {
                    ...
                    countForFunction: this.func.c[1]};
...
class SampleWeightSelectorView {
    constructor(divContainer, eventInfo, onSelectChange) {
        ...
        options.set('percent_to_function',
                    'Show percentage of event count relative to the current function');
        options.set('event_count', 'Show event count');
        if (isClockEvent(eventInfo)) {
            options.set('event_count_in_ms', 'Show event count in milliseconds');
        }
        ...
        this.curOption = 'percent_to_function';
        let id = createId();
        let str = `
            <div class="dropdown">
                <button type="button" class="btn btn-primary dropdown-toggle" id="${id}"
    ...
    getSampleWeightFunction() {
        ...
        if (this.curOption == 'percent_to_thread') {
            return function(eventCount, totalCount) {
                ...
                // 新增下面的 IF 代码块
        if (this.curOption == 'percent_to_function') {
            return function(eventCount, totalCount) {
                // 检查函数对象和其计数数据是否存在
                if (!totalCount.functionObj || !totalCount.functionObj.c) {
                    return '0.00%';
                }
                // 从函数对象中获取“self event count”作为分母
                let functionTotal = totalCount.functionObj.c[2];
                if (functionTotal === 0) {
                    return '0.00%'; // 避免除以零
                }
                let percent = eventCount * 100.0 / functionTotal;
                return percent.toFixed(2) + '%';
            };
        }
...
function collectDisassemblyForFunction(func) {
    ...
    for (let line of rawCode) {
        // --- 去掉汇编源代码行数显示
        if (line[0].trim().startsWith(';') && line[0].trim().includes('.S:'))
            continue;
        // ---------------------
        let code = line[0];
...
class DisassemblyView {
    ...
    realDraw(sampleWeightFunction) {
        ...
        let rows = [];
           // --- MODIFICATION START ---
    // 创建一个辅助函数来格式化和着色百分比
    function formatAndColor(eventCount, totalCount) {
        // 如果事件数为0，直接返回空字符串
        if (eventCount === 0) {
            return '';
        }

        // 调用现有的函数来获取百分比字符串，例如 "1.23%"
        const valueStr = sampleWeightFunction(eventCount, totalCount);

        // 从字符串中解析出数值部分
        const numericValue = parseFloat(valueStr);

        // 如果数值大于等于1，则添加红色和粗体的样式
        if (numericValue >= 1) {
            return `<span style="color: #D32F2F; font-weight: bold;">${valueStr}</span>`;
        }
    ...
    // --- MODIFICATION END ---
        ...
        for (let line of this.disassembly) {
            ...
            //}
        // --- MODIFICATION START ---
        // 使用新的辅助函数来处理 Total 和 Self 的值
        let totalValue = formatAndColor(line.subtreeEventCount, this.totalCount);
        let selfValue = formatAndColor(line.eventCount, this.totalCount);
        // --- MODIFICATION END ---
