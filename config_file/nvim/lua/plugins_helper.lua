...
function M.create_augroup(name) return api.nvim_create_augroup(name, { clear = true }) end
...
function M.nerdcommenter_toggle_visual_keep() fn["nerdcommenter#Comment"]("x", "invert") end
...
local function nerdtree_window()
  for _, win in ipairs(api.nvim_list_wins()) do
    ...
    if vim.bo[buf].filetype == "nerdtree" then return win end
  end
end
...
local function tagbar_window()
  for _, win in ipairs(api.nvim_list_wins()) do
    local buf = api.nvim_win_get_buf(win)
    if vim.bo[buf].filetype == "tagbar" then return win end
  end
end
...
local function layout_nerdtree_and_tagbar()
  ...
  if not tree_win or not tagbar_win then return end
...
function M.open_tagbar_below_nerdtree()
  ...
  local tree_win = nerdtree_window()
  if tree_win then api.nvim_set_current_win(tree_win) end
...
function M.mark_nerdtree_current_file()
  ...
  api.nvim_win_call(tree_win, function()
    ...
    if has_previous then pcall(fn.matchdelete, match_id) end
...
function M.sync_nerdtree_current_file()
  if nerdtree_sync_in_progress then return end
  ...
  if ok then M.mark_nerdtree_current_file() end
  if api.nvim_win_is_valid(source_win) then api.nvim_set_current_win(source_win) end
